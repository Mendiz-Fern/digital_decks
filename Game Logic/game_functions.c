...
int open_serial_player_x(int player){
    char *port = malloc(13);
    char *port_use = port;
    switch(player)
    {
...
void send_nu(int controller, __uint16_t data, int serial_port){
...
__uint16_t recv_nu(int controller, int serial_port){  // Button press code ideas- 
    char readbuff[256];
...
void close_serial_player_x(int serial_port){
    close(serial_port);
}
